...
use integration::test_util::{
    load_block_traces_for_test, parse_trace_path_from_mode,
    PARAMS_DIR,
};
use prover::{
    config::INNER_DEGREE,
...
fn test_cs_same_for_vk_consistent() {
    ...
    let real_circuit = SuperCircuit::from_block_traces(block_trace.clone()).unwrap().0;
...
fn estimate_circuit_rows() {
    ...
    let rows = SuperCircuit::estimate_rows(block_trace.clone());
...
fn test_deterministic() {
    ...
    let circuit2 = C::from_block_traces(block_trace.clone()).unwrap().0;
...
fn test_vk_same() {
    ...
    let real_circuit = C::from_block_traces(block_trace.clone()).unwrap().0;
